...
class NanopolishDS(Dataset):
    ...
    def set_feature_indices(self):
        ...
        '''
        self.total_neighboring_features = self.num_neighboring_features #self.get_total_neighboring_features()
    ...
    def __len__(self) -> int:
        ...
        '''
        return len(self.tmpl) #len(self.data_info)

    #def get_total_neighboring_features(self, idx) -> int:
    #    r'''
    ...
    #    #                                                              "start", "end"]]
    #    tx_id, tx_pos, kmer, features = self._load_data(self.tmpl, idx)
    #    return (len(kmer) - 5) // 2
    ...
    def _load_data(self, tmpl, idx):
        ...
        d=tmpl[idx]
        tx_id=list(d.keys())[0]
        tx_pos=list(d[tx_id].keys())[0]
        kmer=list(d[tx_id][tx_pos].keys())[0]
        features=d[tx_id][tx_pos][kmer]
        ...
        return tx_id, tx_pos, kmer, np.array(features)
    ...
    def __getitem__(self, idx: int) -> Union[Tuple[np.ndarray, str, str, int, np.ndarray],
        ...
        #print(tx_id, tx_pos, read_ids, features, kmer)
