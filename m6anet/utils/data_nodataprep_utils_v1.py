...
class NanopolishDS(Dataset):
    ...
    def __init__(self, tx_ids, blow5: str, fastq: str, bam: str, fasta: str,
    ...
    def load_data(self, idx: int) -> Tuple[str, int, np.ndarray, np.ndarray, str]:
        ...
        cmd='samtools view -b '+self.bam+' '+tx_id+' > '+tx_id+'.bam'
        subprocess.run(cmd, shell=True, check=True)
        cmd='samtools index '+tx_id+'.bam'
        subprocess.run(cmd, shell=True, check=True)
        try:
            cmd='/home/gisuser/softwares/f5c-collapse2/f5c eventalign -r '+self.fastq+' -b '+tx_id+'.bam -g '+self.fasta+' --slow5 '+self.blow5+' -t 1 --signal-index --m6anet --min-mapq 0 | grep '+grep_pattern+' - > '+tx_id+'_eventalign.txt'
            subprocess.run(cmd, shell=True, check=True)
            subprocess.run('rm '+tx_id+'.bam*',shell=True, check=True)
            eventalign_result=pd.read_csv(tx_id+'_eventalign.txt',sep='\t')
            eventalign_result=eventalign_result.iloc[:,[3,1,2,6,7,8,9]]
            eventalign_result.columns=['read_index','position','reference_kmer','event_level_mean','event_stdv','event_length','model_kmer']
            for pos in set(eventalign_result['position']):
                d={tx_id:{}}
                ...
                for read_index in set(pos_df['read_index']):
                    ...
                    if len(tmp_df) == 3:
                        ...
                        entry=[tmp_df['event_length'].iloc[0],tmp_df['event_stdv'].iloc[0],tmp_df['event_level_mean'].iloc[0],
                            ...
                            tmp_df['event_length'].iloc[2],tmp_df['event_stdv'].iloc[2],tmp_df['event_level_mean'].iloc[2]]
